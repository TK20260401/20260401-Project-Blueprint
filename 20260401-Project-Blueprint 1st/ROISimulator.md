...
19. **印刷/PDF機能強化**
    - シミュレーション作成画面（/simulations/new）にも印刷/PDFボタン追加
    - 印刷用CSS強化（SVGグラフ・背景色・テーブル罫線の確実表示）
    - 作成画面: 印刷時に入力パネル非表示、グラフ+コスト内訳テーブル+前提条件を出力
    - 印刷用ヘッダー（タイトル・企業名・日付）・フッター（Confidential）
    - 詳細画面・作成画面の両方からPDF保存可能
